...
export async function replace(this: SBTree, currentDocument, newDocument) {
  ...
  for (const _addedFieldName in addedFields) {
    ...
    if (validTypes.includes(_addedFieldType)) {
      ...
      this.verbose &&
        console.log(
          `No index for ${_addedFieldName} : Typeof ${_addedFieldType} : ${JSON.stringify(
            _addedFieldValue,
          )}`,
        )
    }
  ...
  for (const _deletedFieldName in deletedFields) {
    ...
    if (validTypes.includes(_deletedFieldType)) {
      ...
      this.verbose &&
        console.log(
          `No index for ${_deletedFieldName} : Typeof ${_deletedFieldType} : ${JSON.stringify(
            _deletedFieldValue,
          )}`,
        )
    }
  ...
  const replaceProp = async (_fieldName, _fieldValue) => {
    ...
    if (['number', 'string', 'boolean'].includes(_fieldType)) {
      ...
      for (const _nestedFieldName in _fieldValue) {
        ...
        await replaceProp(
          `${_fieldName}.${_nestedFieldName}`,
          _fieldValue[_nestedFieldName],
        )
      }
    } else {
  ...
  for (const _changedFieldName in changedField) {
    ...
    if (validTypes.includes(_changedFieldType)) {
      ...
      this.verbose &&
        console.log(
          `No index for ${_changedFieldName} : Typeof ${_changedFieldType} : ${JSON.stringify(
            _changedFieldValue,
          )}`,
        )
    }
