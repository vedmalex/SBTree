...
export async function query(this: SBTree, query): Promise<Array<Document>> {
  const findNested = async (_promises, _queryFieldName, _queryFieldValue) => {
    for (const nestedQueryFieldName in _queryFieldValue) {
      ...
      if (
        ['number', 'string', 'boolean', 'object'].includes(nestedQueryFieldType)
      ) {
        const fTree = this.getFieldTree(
          `${_queryFieldName}.${nestedQueryFieldName}`,
        )
        ...
        nestedQueryFieldType === 'object' &&
        !Array.isArray(nestedQueryFieldValue)
      ) {
        await findNested(
          _promises,
          `${_queryFieldName}.${nestedQueryFieldName}`,
          nestedQueryFieldValue,
        )
      } else {
    ...
    switch (queryFieldType) {
      ...
      case 'object':
        if (Array.isArray(queryFieldValue)) {
          throw new Error(
            'Not supported array input. Please open a Github issue to specify your need.',
          )
        } else {
          const operators = Object.keys(queryFieldValue).filter(
            (el) => el[0] === '$',
          )
  ...
  return resolveDocuments.call(this, matchingObjectIds) as ReturnType<
    typeof resolveDocuments
  >
}
