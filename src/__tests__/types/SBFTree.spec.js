...
describe('SBFTree', () => {
  ...
  it('should instantiate', async function () {
    ...
    expect(Object.keys(tree.root.children).length).to.deep.equal(3);

    expect(tree.root.children[0].keys).to.deep.equal([10]);
    expect(Object.keys(tree.root.children[0].children).length).to.deep.equal(2);

    const leafs = tree.getAdapter().leafs;
    const bucket1Id = tree.root.children[0].children[0].id;
    expect(leafs[bucket1Id].data.keys).to.deep.equal([0]);

    const bucket2Id = tree.root.children[0].children[1].id;
    expect(leafs[bucket2Id].data.keys).to.deep.equal([10, 20]);

    expect(tree.root.children[1].keys).to.deep.equal([40]);
    expect(Object.keys(tree.root.children[1].children).length).to.deep.equal(2);

    const bucket3Id = tree.root.children[1].children[0].id;
    expect(leafs[bucket3Id].data.keys).to.deep.equal([30]);

    const bucket4Id = tree.root.children[1].children[1].id;
    expect(leafs[bucket4Id].data.keys).to.deep.equal([40, 50]);

    expect(tree.root.children[2].keys).to.deep.equal([70, 80]);
    expect(Object.keys(tree.root.children[2].children).length).to.deep.equal(3);

    const bucket5Id = tree.root.children[2].children[0].id;
    expect(leafs[bucket5Id].data.keys).to.deep.equal([60]);

    const bucket6Id = tree.root.children[2].children[1].id;
    expect(leafs[bucket6Id].data.keys).to.deep.equal([70]);

    const bucket7Id = tree.root.children[2].children[2].id;
    ...
    expect(tree.root.children[0].keys).to.deep.equal([30]);
    expect(tree.root.children[0].children[0].keys).to.deep.equal([10]);
    expect(tree.root.children[0].children[1].keys).to.deep.equal([40]);
    expect(tree.root.children[1].keys).to.deep.equal([80]);

    expect(tree.root.children[1].children[0].keys).to.deep.equal([70]);
    expect(tree.root.children[1].children[1].keys).to.deep.equal([80]);
  });
});
